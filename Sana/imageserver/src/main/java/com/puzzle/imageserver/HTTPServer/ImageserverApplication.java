...
public class ImageserverApplication {
     ...
     * @throws IllegalArgumentException if imageName is null, empty, or does not meet the specified
     ...
     * @throws ResponseStatusException if an error occurs while reading the image file.
     */
    private byte[] readImageBytes(File imageFile) throws ResponseStatusException {
        ...
        // 1.5 imageFile.size is very large and there is not enough memory available to store the returned array.
        // 
        // Test Cases:
        // imageFile.bmp size = 10 MB (covers 1.1 & 1.4).
        // imageFile.png size = 917 KB (covers 1.2 & 1.4).
        // imageFile.JPEG size = 1  MB (covers 1.3 & 1.4).
        // call this method 100 times to test for memory leaks should throw ResponseStatusException (covers 1.5).
    }

    /**
     * Retrieves the response image with the given imageName and returns it as a byte array
     * 
     * @param imageName A string representing the name of the image file to retrieve 
     * @return {ResponseEntity<byte[]>} a ResponseEntity containing the byte array
     *           as the response body and the appropriate HTTP headers for the image file type
     * @throws ResponseStatusException if the image file does not exist or an error occurs while
     *        reading its content.
     * @throws IllegalArgumentException if imageName is empty, contains any special characters
     *         other than hyphens and periods.
     * @throws RuntimeException if an unexpected runtime exception occurs while attempting to load
     *         the image file.
     */
    @GetMapping(value = "/{imageName}", produces = MediaType.IMAGE_JPEG_VALUE)
    public ResponseEntity<byte[]> getImage(@PathVariable String imageName) throws ResponseStatusException, IllegalArgumentException, RuntimeException {
        try {
        ...
        // 1.6 imageBytes is huge and there is not enough memory available to store the returned array.
        ...
        //             Supercalifragilisticexpialidocious-Supercalifragilisticexpialidocious-
        //             Supercalifragilisticexpialidocious.png  (covers 1.2).
        // imageName = SE324-Project covers 0 < imageName.Length < 255 (covers 1.3 & 1.4 & 1.5 ).
        // imageName = IMAGE must return the same result when we have imageName = image covers 
